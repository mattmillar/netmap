...
class dataface_actions_forgot_password {
		...
		try {
			if ( isset($query['--uuid']) ){
				...
				if ( $res ){
					...
					throw new Exception("Failed to reset password for uuid ".$query['--uuid']);
				}
				...
				if ( @$query['--format'] == 'json' ){
					$this->response(array(
						'code'=>200,
						'message'=>'An email has been sent to the provided email address with instructions for resetting your password.'
					));
					exit;
				} else {
				...
				if ( @$query['--format'] == 'json' ){
					$this->response(array(	
					'code'=>200,
					'message'=> 'An email has been sent to the email on file for this user account with instructions for resetting the password.'
					));
					exit;
				} else {
					df_display(array(), 'xataface/forgot_password/sent_email.html');
	...
	function send_reset_email_for_username($username){
		...
		if ( !$usernameCol ) throw new Exception("No username Column found in the users table.  Please specify one using the username_column directive in the [_auth] section of the conf.ini file.", self::$EX_NO_EMAIL_COLUMN_FOUND);
		
		$people = df_get_records_array($auth->usersTable, array($usernameCol => '='.$username));
		if ( !$people ) throw new Exception("No account found with that username", self::$EX_NO_USERS_FOUND_WITH_USERNAME);
		if ( count($people) > 1 ){
			throw new Exception("Multiple users found with same username", self::$EX_MULTIPLE_USERS_WITH_SAME_USERNAME);
	...
	function send_reset_email_for_email($email){
		...
		if ( !$emailCol ) throw new Exception("No Email Column found in the users table.  Please specify one using the email_column directive in the [_auth] section of the conf.ini file.", self::$EX_NO_EMAIL_COLUMN_FOUND);
		
		$people = df_get_records_array($auth->usersTable, array($emailCol => '='.$email));
		if ( !$people ) throw new Exception("No account found with that email address", self::$EX_NO_USERS_WITH_EMAIL);
		if ( count($people) > 1 ){
			throw new Exception("Multiple users found with same email address", self::$EX_MULTIPLE_USERS_WITH_SAME_EMAIL);
	...
	public function send_reset_email_for_user(Dataface_Record $user){
		...
		if ( !$emailCol ) throw new Exception("No Email Column found in the users table.  Please specify one using the email_column directive in the [_auth] section of the conf.ini file.", self::$EX_NO_EMAIL_COLUMN_FOUND);
		if ( !$usernameCol ) throw new Exception("No username column found in the users table. Please specify one using the username_column directive in the [_auth] section of the conf.ini file.", self::$EX_NO_USERNAME_COLUMN_FOUND);
		if ( !$user ) throw new Exception("Cannot send email for null user", self::$EX_NO_USERS_FOUND_WITH_EMAIL);
		...
		if ( !$email ) throw new Exception("User has not email address on file", $EX_NO_EMAIL_FOR_USER);
		...
		if ( !$row ) throw new Exception("Failed to fetch reset password request row from database after it has been inserted.  This should never happen ... must be a bug");
		
		$uuid = $row['request_uuid'];
		if ( !$uuid ) throw new Exception("Blank uuid for the reset request.  This should never happen.  Must be a bug.");
...
END;
	...
	public function reset_password_with_uuid($uuid){
		...
		if ( !$res ) throw new Exception(mysql_error(df_db()));
		$row = mysql_fetch_assoc($res);
		if ( !$row ) throw new Exception("No such reset request could be found", self::$EX_NO_SUCH_UUID);
		
		if ( !$row['username'] ){
			throw new Exception("Attempt to reset password for user with null username", self::$EX_NO_USERNAME_FOR_USER);
		...
		if ( !$user ){
			throw new Exception("No user account found with that username", self::$EX_USER_NOT_FOUND);
			
		}
		$emailColumn = $auth->getEmailColumn();
		if ( !$emailColumn ) throw new Exception("No email column found in the users table", self::$EX_NO_EMAIL_COLUMN_FOUND);
		$email = $user->val($emailColumn);
		
		if ( !$email ){
			throw new Exception("User has account has no email address on record.  Please contact support to reset the password", self::$EX_NO_EMAIL_FOR_USER);

...
class Dataface_CSSTool {
	...
	public function import($path){
		if ( @$this->ignoreFiles[$path] ) return;
	...
	private function generateCacheKeyForScripts($stylesheets){
		//$this->sortScripts();
		$base = basename($stylesheets[0]);
		//$base = basename($scripts[0]);
		$base = substr($base, 0, 10);
		return $base.'-'.md5(implode(PATH_SEPARATOR, $stylesheets));
	}
	
	private function writeCSS($stylesheets, $contents){
		$path = $this->getCSSCachePath($stylesheets);
	...
	private function writeManifest($stylesheets, $included){
		$path = $this->getManifestPath($stylesheets);
		return file_put_contents($path, json_encode($included), LOCK_EX);
	}
	...
	private function isCacheDirty($stylesheets){
		...
		$deps = json_decode(file_get_contents($mfpath), true);
	...
	public function compile($stylesheets=null, $clean=false){
		if (!isset($stylesheets) ){
			$stylesheets = array_keys($this->stylesheets);
		}
		
		if ( $clean or $this->isCacheDirty($stylesheets) ){
			$this->included = array();
			$contents = $this->_compile($stylesheets);
			...
			file_put_contents($this->getCSSCachePath($stylesheets), $contents, LOCK_EX);
			file_put_contents($this->getManifestPath($stylesheets), json_encode($this->included), LOCK_EX);
		}
	...
	/**
	 * Compiles a set of CSSs specified byt the $stylesheets array.
	 * This respects the //!require and //!include directives
	 * which includes other CSS files inline.
	 *
	 * @param mixed $stylesheets  Either a string with a path to a script,
	 ...
	 *		
	 */
	private function _compile($stylesheets){
		//$included = array();
		$out=array();
